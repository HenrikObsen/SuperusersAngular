...
export class DataService {
  private loggingService = new LogService()

  private address:string = "assets/database/data.json"

  private headers = new Headers({ 'Content-Type': 'application/json', 'charset': 'UTF-8' });
  private options = new RequestOptions({ headers: this.headers });

  constructor(private http: Http, private logs : LogService) { 
    logs.log("her logges alt centralt")
  }

  hentData(): Observable<any> {
        
    return this.http.get(this.address)
        .map(res => res.json())
        .catch( err =>  [ this.herLoggesAlt(err) ] )
  ...
  hentData_via_jQuery() : Promise<any>{
  	  	return jQuery.getJSON(this.address)
  }

  hentData_via_fetch() : Promise<any>{
  	  	return fetch(this.address)
  }

  //kun for at bygge data.json fil
  hentRawData() {
  	return this.rawData()
  }

  // bruges ikke til løsning mere. må gerne slettes nu :-)
  rawData() : Array<any> {
  	 return [
    {
      id: 1,
